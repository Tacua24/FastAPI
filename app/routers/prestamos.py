...
def generar_prestamo(prestamo: PrestamoCrear, db: Session = Depends(get_db)):
    estudiante = (
        ...
        .filter(EstudianteModel.id == prestamo.estudiante_id)
        .first()
    )
    if not estudiante:
    ...
    if not libro.disponible:
        raise HTTPException(status_code=404, detail="El libro ya se encuentra prestado")

    libro.disponible = False
    nuevo_prestamo = PrestamoModel(
        libro_id=prestamo.libro_id, estudiante_id=prestamo.estudiante_id
    )
